...
import br.com.cicom.comunicacicom.DSPrimary.repository.importacao.Cicom1LogRepository;

@Service
public class Cicom1LogService extends CecocoLog {

	@Autowired
	private Cicom1LogRepository repositorio;

	public List<CecocoLog> buscaTodosPorNumeroocorrencia(Long numeroocorrencia) {
		this.repositorio.flush();
		List<Cicom1Log> listacicomLog = new ArrayList<Cicom1Log>();
		List<CecocoLog> listaCecocoLog = new ArrayList<>();
		listacicomLog =  repositorio.findByNumeroocorrencia(numeroocorrencia);

		for(Cicom1Log cicomLog : listacicomLog) {

...
import br.com.cicom.comunicacicom.DSPrimary.repository.importacao.Cicom20LogRepository;

@Service
public class Cicom20LogService extends CecocoLog {

	@Autowired
	private Cicom20LogRepository repositorio;

	public List<CecocoLog> buscaTodosPorNumeroocorrencia(Long numeroocorrencia) {
		this.repositorio.flush();
		List<Cicom20Log> listacicomLog = new ArrayList<Cicom20Log>();
		List<CecocoLog> listaCecocoLog = new ArrayList<>();
		listacicomLog =  repositorio.findByNumeroocorrencia(numeroocorrencia);

		for(Cicom20Log cicomLog : listacicomLog) {

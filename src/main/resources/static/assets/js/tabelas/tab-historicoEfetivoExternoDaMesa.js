function gerarTabela(idMesa){
		...
		"ajax": '/policiamento/buscar/historico/'+idMesa,
		"order" : [ [ 0, "desc" ] ],	
		"columns": [
				...
				{					
					data :'mesa.nome',
					name: 'Mesa',render: function converter(data, type, row){
						return row.mesa;
					},
					searchable: true
				},
				...
				
				{
		            ...
		            render: function adicionaBotaoAcao(data, type, full, meta){
		            	...
		            	var botaoAcao = '<div class="btn-group" role="group" aria-label="Button group with nested dropdown">'+
						   ...
						   '				<a class="dropdown-item" target="_blank" href="/admin/cadastros/report/efetivoExterno/'+ full.id +'" id="'+ full.id +'"><i class="fas fa-print"></i> Imprimir</a>'+
						   '			</div>'+
						   '		</div>'+
						   '	</div>'

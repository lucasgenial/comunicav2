$(document).ready(function() {
	$('#tabela-ocorrencia-paginada').DataTable({
		"columnDefs" : [ {
			"targets" : 9,
			"orderable" : true
		} ],
...
//					return date;
			...
			},
			{
				...
				render: function toLimit(descricao, type, row){
					return descricao.substring(0,44);
				}
			},
			{ 
				...
				name: 'Cidade',
					render: function converter(data, type, row){
						return row.endereco.cidade.nome;
					}
			},
			{
				...
				name: 'Localidade',
				render: function converter(data, type, row){
					return row.endereco.bairro.localidade.nome;
				}
			},
			{
				...
				name: 'Bairro',
					render: function converter(data, type, row){
						return row.endereco.bairro.nome;
					}
			},
			{
				data : 'enviada',
			...
			},
			{
				...
				width: "5%",
				render: function converter(data, type, row){
					return row.estabelecimento.nome;
				}
			}
		],
	});
				...
				},
				{
		            ...
		            render: function adicionaBotaoAcao(data, type, full, meta){
		            	...
		            	var botaoAcao = '<div class="btn-group" role="group" aria-label="Button group with nested dropdown">'+
						   ...
						   '				<a class="dropdown-item" target="_blank" href="/admin/visualiza/ocorrencias/'+ full.id +'" id="'+ full.id +'"><i class="glyphicon glyphicon-eye-open"></i> Visualizar</a>'+
						   ...
						   '				<a class="dropdown-item" href="javascript:enviarWhatsappOcorrenciaViaLink('+ full.id +')"><i class="fab fa-whatsapp"></i> Via Link</a>'+
...
/* ENVIO DE OCORRÊNCIA VIA LINK */
// /admin/cadastros/ocorrencias/visualizarWhatsapp/externo/{id}(id=${_ocorrencia.id})}
function enviarWhatsappOcorrenciaViaLink(id){
...
function enviarEmailocorrencia(id){
	...
	//var id = this.id;
		...
		$.ajax({
		      ...
		      $('#formularioEmail').attr("action", "/admin/cadastros/ocorrencias/enviarEmail/"+id);
		      }
			});
